...
function main()
    autoupdate("https://raw.githubusercontent.com/rIIvenRvT/rvt/main/update.json", '['..string.upper(thisScript().name)..']: ', "https://www.blast.hk/threads/85370/")
            ...
            if f then
              ...
              if updateversion ~= thisScript().version then
                lua_thread.create(function(prefix)
                  local dlstatus = require('moonloader').download_status
                  local color = -1
                  sampAddChatMessage((prefix..'Atualização detectada. estou tentando atualizar c '..thisScript().version..' на '..updateversion), color)
                  wait(250)
                  downloadUrlToFile(updatelink, thisScript().path,
                    function(id3, status1, p13, p23)
                      if status1 == dlstatus.STATUS_DOWNLOADINGDATA then
                        print(string.format('carregado %d a partir de %d.', p13, p23))
                      elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                        print('Download da atualização concluído.')
                        sampAddChatMessage((prefix..'Atualização completa!'), color)
                        goupdatestatus = true
                        lua_thread.create(function() wait(500) thisScript():reload() end)
                      end
                      if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                        if goupdatestatus == nil then
                          sampAddChatMessage((prefix..'A atualização falhou. Estou executando uma versão desatualizada..'), color)
                          update = false
                        end
                      end
                    end
                ...
                update = false
                print('v'..thisScript().version..': Nenhuma atualização necessária.')
              end
            end
          else
            print('v'..thisScript().version..': Não consigo verificar se há atualização. Aceite ou verifique-se '..url)
            update = false
          end
        end
      end
